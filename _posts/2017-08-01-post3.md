...
These notes are the result of the interplay between my mathematical upbringing and my development as a theoretical biologist. I have placed them here primarily for my own reference. As I continue to learn and explore I will update these notes with my findings, compiling not only a series of tidy results, but hopefully also a unifying mathematical framework. The ideas and calculations discussed here form the backbone of my understanding of evolutionary and ecological theory.
...
Let's suppose there are $N_t$ different individuals in the population at time $t$, with each individual $i$ having fitness $\mathcal{W_t}(i)$. If we further suppose that generations are non-overlapping, then we obtain the population size in the next generation as $N_{t+1}=\sum_{i=1}^{N_t}\mathcal{W_t}(i)$. If each $\mathcal{W_t}(i)$ is a Poisson random variable with mean $W_t(i)$ (in symbols this is written $\mathcal{W_t}(i)\sim\mathcal{P}(W_t(i))$), then by the additive property of Poisson random variables we have $N_{t+1}\sim\mathcal{P}\left(\sum_{i=1}^{N_t}W_t(i)\right)$. If selection is absent so that each $\mathcal{W_t}(i)$ is identically distributed with mean $W_t$, then $N_{t+1}\sim\mathcal{P}(W_tN_t)$. 
...
$$g_1(z)g_2(z)=Cg_3(z), \text{ where } \mu_3=\frac{\sigma_1^2\mu_2+\sigma_2^2\mu_1}{\sigma_1^2+\sigma_2^2},
 \ \sigma_3^2=\frac{\sigma_1^2\sigma_2^2}{\sigma_1^2+\sigma_2^2}, \ C=\int g_1g_2dz=\frac{1}{\sqrt{2\pi(\sigma_1^2+\sigma_2^2)}}\exp\left(-\frac{(\mu_1-\mu_2)^2}{2(\sigma_1^2+\sigma_2^2)}\right).$$
...
C'=\int\prod_{i=1}^{n-1}g_idz$$
 
$$(g_1*g_2)(z)=g_3(z), \text{ where } \mu_3=\mu_2+\mu_1, \ \sigma_3^2=\sigma_1^2+\sigma_2^2.$$

## More On Gaussians

Now that we have solved the mystery of the mean fitness this might be a good time to review <a href="http://onlinelibrary.wiley.com/doi/10.1111/j.1558-5646.1976.tb00911.x/abstract">that 1976 classic</a>. Otherwise, I have a few more tricks with Gaussians that have come in handy during my journeys. Something that I noticed about our popular little friends is that they like to stay Gaussian. Doing things to them that perturb their normality makes them recoil with disgust, causing symbols to combust from the paper upon which they were written. I have found that facilitating their Gaussianness has made my life much easier. One manifestation of this principle comes up when choosing a fitness function. <a href="http://ida.lib.uidaho.edu:3278/doi/10.1111/j.1558-5646.2009.00909.x/full">It's been shown that</a> that a formula, which is identical to Bayes theorem, provides basically the most all around correct way of calculating the post-selection phenotypic distribution, the distribution of phenotypes left over after selection but before reproduction. In our notation it is

$$\tilde\phi(z)=\frac{\phi(z)W(z)}{\int\phi Wdz}.$$

As we saw above, $\tilde\phi$ will remain Gaussian if $W$ is also Gaussian. The the version of the breeder's equation used in this note depends very much on the normality of the parental population. If we want to use the breeder's equation to predict the mean phenotype of future generations, then the offspring populations will also need to be distributed normally. Since $\tilde\phi$ is our parental phenotypic distribution, we want to choose a fitness function $W$ that preserves normality. As you may have guessed, it's possible to show that if $\tilde\phi$ and $\phi$ are Gaussian, then so is $W$. To do so just solve for $W$ in the above equation. 

However, some basic fitness functions are clearly not Gaussian such as the sigmoid representing directional selection or a parabola opening up representing diversifying selection. For some of these, a Gaussian approximation can work quite well. For the sigmoid one could instead set $\theta=\bar z + \delta$ as the phenotypic optimum in the Gaussian fitness used above for approximately the same effect. Since the mean fitness using the sigmoid is difficult to calculate and deal with if it can be calculated, a linear approximation is usually invoked which results in the same dynamics with just using the aformentioned Gaussian and setting $\delta=\pm\omega$ with the sign depending on the directionality of the sigmoid. 

On the other hand, approximating diversifying selection has no easy solution. The reason for this is that diversifying selection perturbs normality in a very fundamental way. We might be able to approximate the parabola decently by adding two normal densities together so that their means are far enough away from each other to make the result bimodal. In this case we can calculate $\tilde\phi$ and it would be the sum of two other Gaussians, which not what we want. To my knowledge there is no decent approximation of diversifying selection that preserves normality. There is, however, one sneaky trick I've employed that involves discontinuous dynamics. To perform this trick we take a Gaussian approximation to directional selection as mentioned above but instead of $\delta$ we use $\text{sgn}(\bar z-\theta)\delta$ with $\text{sgn}(x)=\frac{x}{\|x\|}$ and center it on $\theta$ instead of the mean phenotype. Here $\theta$ no longer reflects the optimal phenotype. This dirty trick can be used to provide a model of diversifying selection that is compatible with the breeder's equation, but is a very desparate attempt to marry the two. It should be noted here that the perturbation of normality by diversifying selection is a topic that has had some debate attached to it some time ago. The conclusion drawn from this debate is that with weak selection and random mating, the perturbation from normality is negligible. I find this conclusion quite insatisfactory. Diversifying selection obviously has the potential to produce non-normal distributions and the mathematics of this situation should be developed as thoroughly and rigorously as possible, not swept under the rug as though it were some technical annoyance.
