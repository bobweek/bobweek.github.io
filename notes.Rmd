...
$$\E(N_t)=\para{\oint_0^tW_s^{ds}}N_0 \text{ and } \Var(N_t)=\para{1+\int_0^{t-1}W_sds}\para{\oint_0^tW_s^{ds}}N_0$$

where $\oint$ represents type 1 product integration. Note that 

$$\oint_0^tW_s^{ds}\equiv e^{\int_0^t\ln(W_s)ds}=e^{\int_0^t r(s)ds}.$$

Hence, this returns the same expected dynamics as above when $\ln(W_t)=r(t)$.

Continuing our $NB$ fit we would set $p_t=1-\para{1+\int_0^{t-1}W_sds}^{-1}$ and $n_t=\frac{1-p_t}{p_t}\para{\oint_0^tW_s^{ds}}N_0=\para{\int_0^{t-1}W_sds}^{-1}\para{\oint_0^tW_s^{ds}}N_0$.

## Populations of Types

In terms of modelling, a population can be very abstract. Its concept is not restricted to a set of living things, as in the biological concept of a population. Here a population can be a set of biological populations, a set of species, and so on. The members of the population may not be uniform, but can consist of various different types. For example, we can consider populations of individuals categorized by their phenotype or populations of species categorized by clades. We derive the Price equation to obtain how the distribution of types within a population fluctuates due to their fitness'.
