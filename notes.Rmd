...
If we assume that mean fitness is constant with respect to time, we can derive continuous time mean dynamics of $N_t$. To do so, suppose that for $0<\delta<1$, $\mathbb{E}N_{t+\delta}=W^\delta N_t$. Then $\mathbb{E}N_{t+0}=N_t$ and $\mathbb{E}N_{t+1}=WN_t$. Hence

$$\frac{d}{dt}\mathbb{E}N_t=\lim_{\delta\rightarrow0}\frac{\mathbb{E}N_{t+\delta}-N_t}{\delta}=\lim_{\delta\rightarrow0}\frac{(W^\delta-1)}{\delta}N_t=\ln(W)N_t$$
