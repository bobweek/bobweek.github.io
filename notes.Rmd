...
Consider a population of $T$ different types, each type $i$ with $n_i(t)$ individuals at time $t$. If we denote $\phi_i(t)$ to be the proportion of individuals within type $i$ at time $t$ among the population, we have $\phi_i(t)=\frac{n_i(t)}{N_t}$ where $N_t=\sum_{i=1}^Tn_i(t)$. Denote the expected fitness of each individual within type $i$ as $W_i$. Then

$$\phi_i(t+1)=\frac{n_i(t)W_i}{\sum_{i=1}^Tn_i(t)W_i}=\frac{\phi_i(t)W_i}{\sum_{i=1}^T\phi_i(t)W_i}=\para{\frac{W_i}{\overline{W}_t}}\phi_i(t)$$

Using the same approach as above for the continuous time expected population dynamics, we can easily get the continuous time dynamics of $\phi_i$. We suppose that for $0<\delta<1$, $\phi_i(t+\delta)=\para{\frac{W_i}{\overline W(t)}}^\delta\phi_i(t)$. Then

$$\frac{d}{dt}\phi_i(t)=\lim_{\delta\rightarrow0}\frac{\phi_i(t+\delta)-\phi_i(t)}{\delta}=\phi_i(t)\para{\ln(W_i)-\ln(\overline W_t)}.$$
...
$$N_{t+1}=\sum_{i=1}^Tn_i(t+1)=\sum_{i=1}^Tn_i(t)W_i=\frac{N_t}{N_t}\sum_{i=1}^Tn_i(t)W_i=N_t\sum_{i=1}^T\phi_i(t)W_i=\overline{W_t}N_t.$$

For readability, we adopt the notation $D_t$ in place of $\frac{d}{dt}$.

$$D_t\phi_i(t)=D_t\frac{n_i(t)}{N_t}=\frac{N_tD_tn_i(t)-n_i(t)D_tN_t}{N_t^2}$$

$$=\frac{\ln(W_i)n_i(t)-n_i(t)\ln(\overline{W}_t)}{N_t}=\phi_i(t)\para{\ln(W_i)-\ln(\overline W_t)}.$$
