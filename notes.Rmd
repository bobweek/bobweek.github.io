...
However, this result is a bit of an abuse of the above derivation as we had made the assumption that fitness is constant with respect to time. If we assume that $\mathbb{E}\frac{dN_t}{dt}=\ln(W_t)N_t$, where now $W_t$ is allowed to change with respect to time, then for consistency it would be good to reconcile this equation with the above assumption that $\mathbb{E}\Delta N_t=\mathbb{E}N_{t+1}-N_t=(W_t-1)N_t$. Doing so restricts the nature of $W_t$, specifically $\frac{dW_t}{dt}=W_t\ln\left(\frac{W_{t+1}}{W_t}\right)$. This states that if $W_t=0$ for some $t$, then $W_s=0$ for $s>t$. That is to say, if the population goes extinct, it remains extinct. This makes perfect sense in the absence of immigration. But there's another implication in this equation and to explore it I will assume that the general form of $W_t$ is $W_t=e^{f(t)}$ for some function $f(t)$. Note that this implies $\mathbb{E}\frac{dN_t}{dt}=f(t)N_t$. Then we can use the previously derived equation to show that $\frac{d}{dt}f(t)=f(t+1)-f(t)$. In the case that $f(t+1)=k+f(t)$ for some constant $k$, we get $f(t)=kt+c$, making $W_t=e^{kt+c}$ and hence $\mathbb{E}\frac{dN_t}{dt}=(kt+c)N_t$. But we can also make $k(t)$ a function $t$. Here we get 

$$f(t)=\int_0^tk(s)ds+c\implies\mathbb{E}\frac{dN_t}{dt}=\left(\int_0^tk(s)ds+c\right)N_t$$
This might be handy if, say, you wanted to incorporate the effects of population dynamics of another species that interacts with this one. In that case $k(t)$ would likely be a function of the other species' population size at time $t$. But there are other possibilities. For example, we can set $f(t+1)=kf(t)$ to get $f(t)=e^{(k-1)t}+c$ and hence $\mathbb{E}\frac{dN_t}{dt}=\left(e^{(k-1)t}+c\right)N_t$ or, of course, $f(t+1)=k(t)f(t)$ to get 

$$\mathbb{E}\frac{dN_t}{dt}=\left(e^{\int_0^tk(s)ds-t}+c\right)N_t.$$

Regardless of what is chosen for $f(t+1)$, this trick will be assured to work when $f(t+1)$ is a linear function of $f(t)$. That is, $f(t+1)=a(t)+b(t)f(t)$. In this general case we get $f(t)=\int_0^ta(s)ds+e^{\int_0^tb(s)ds-t}+c$ and hence

$$\mathbb{E}\frac{dN_t}{dt}=\left(\int_0^ta(s)ds+e^{\int_0^tb(s)ds-t}+c\right)N_t.$$
...
=\frac{\overline W+(1-p)\left(W_1-W_2\right)}{\overline W-p\left(W_1-W_2\right)}=\frac{W_1}{W_2}$$
