...
Picking up where we left off, the variance has now been decoupled from the expectation forcing future generations population size distributions to be non-Poisson. This can be rectified if we choose the next best option$\dots$ the negative binomial $\mathrm{NB}(r,p)$. By setting $r=WN_0$ and $p=1-\frac{1}{1+W}$ this provides the expectation and variance derived above, but is $\mathrm{NB}$ the actual distribution of $N_2$? Regardless, we can continue this program to derive distributions for future population sizes. In general, we get 

$$\E(N_t)=W^tN_0 \text{ and } \Var(N_t)=\para{\sum_{i=0}^{t-1}W^i}W^tN_0=\frac{1-W^t}{1-W}W^tN_0.$$

Our negative binomial for each time $t$ can be accordingly parameterized by $r_t=\frac{1-W}{W-W^t}W^tN_0$ and $p_t=\frac{W-W^t}{1-W^t}$

HYPOTHESIS: For $W_t\neq W$, $\E(N_t)=N_0\prod_{i=1}^tW_i$ and $\Var(N_t)=N_0\para{\prod_{i=1}^tW_i}\para{1+\sum_{i=1}^{t-1}W_i}$

and for continuous time $\E(N_t)=N_0\oint_0^tW_s^{ds}$ and $\Var(N_t)=N_0\para{\oint_0^tW_s^{ds}}\para{1+\int_0^{t-1}W_sds}$ where $\oint$ represents type 1 product integration. Note that $\oint_0^tW_s^{ds}=e^{\int_0^t\ln W_sds}=e^{\int_0^t r(s)ds}$
