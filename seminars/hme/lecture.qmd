---
title: Modelling Host-Microbiome Evolution
...
![[Andrew Rae for Quanta Magazine](https://www.quantamagazine.org/should-evolution-treat-our-microbes-as-part-of-us-20181120/)](images/microbial-world.jpg){width=800}
</div>
...
.grid-2x2 {
  ...
  gap: 3rem;
  row-gap: 1rem;        /* keeps vertical spacing larger */
  width: 100%;
}
...
<div>
<img src="images/stomach.jpg">
<p>metabolism / digestion / nutrition</p>
</div>

<div>
<img src="images/sad-plant.webp">
<p>immune response / pathogen defense</p>
</div>

<div>
<img src="images/bee-lifecycle.jpg">
<p>development / life-history / phenology</p>
</div>

<div>
<img src="images/brain.webp">
<p>behavior / neurophysiology</p>
</div>
...
<div class="r-stack" style="text-align:center;">
![Modified from Henry et al. (2021), Nat. Comm.](images/henryetal.png){width=800}
</div>
...
<div class="r-stack" style="text-align:center;">
![Modified from Morris & Bohannan (2024), Nat. Micr.](images/mbiome-herit.png){width=500}
</div>
...
- **Magnitude of heritability:** *~0.01–0.5*
...
<div class="r-stack" style="text-align:center;">
![Modified from <br> Morris & Bohannan (2024), Nat. Micr.](images/mbiome-inhr-proc.png){width=600}
</div>
...
a. Direct parent-offspring transmission
<br><br>
b. Host genes associated with microbes
<br><br>
...
<div class="r-stack" style="text-align:center;">
![Modified from Uller & Helanterä (2017), Extended Evolutionary Synthesis blog](images/lewontin-t.png){width=800}
</div>

<!-- - **Lewontins Conditions** $\uparrow$ for evolutionary adaptation

- mebe another slide or two here ? -->
...
<br>
<div class="r-stack" style="text-align:center;">
![Modified from Miller, Svanbäck & Bohannan (2018), Trends Ecol. Evol.](images/metacomm.png){width=400}
</div>
...
<div class="r-stack" style="text-align:center;">
![Modified from <br> Morris & Bohannan (2024), Nat. Micr.](images/mbiome-ext-inhr.png){width=600}
</div>
...
<ol type="a">
  <li value="1">Maternal effect</li><br><br>
  <li value="3">Indirect genetic effect</li><br><br>
  <li value="5">Niche construction</li>
</ol>
...
<div class="r-stack" style="text-align:center;">
![Modified from <br> Week et al. (2024), Nat. Evo. Eco.](images/me.png){width=500}
</div>
...
<div class="r-stack" style="text-align:center;">
![Modified from <br> Week et al. (2024), Nat. Evo. Eco.](images/ige.png){width=750}
</div>
...
<div class="r-stack" style="text-align:center;">
![Modified from <br> Week et al. (2024), Nat. Evo. Eco.](images/nc.png){width=700}
</div>
...
<div class="r-stack" style="text-align:center;">
  ![Modified from Day and Bonduriansky (2011), Am. Nat.](images/uni.png){width=50%}
</div>
...
<div class="r-stack" style="text-align:center;">
![Modified from Roughgarden (2023), Am. Nat](images/lin-coll.png){width=1200}
</div>
...
> **Gap:** Connect concepts of *microbiome inheritance* and *host trait variance*
to predict *microbiome-mediated adaptation*
...
<div class="r-stack" style="text-align:center;">
<span style="font-size: 0.7em;">*microbiome inheritance*</span>
</div>
<div class="r-stack" style="text-align:center;">
![](images/lin-coll-crop.png){width=800}
</div>
...
<div class="r-stack" style="text-align:center;">
<span style="font-size: 0.7em;">*host trait adaptation*</span>
</div>
...
# observed genotype-specific marginal phenotypes
df_pts <- data.frame(
  genotype = c(0, 1, 2),
  y_obs    = c(-0.125, 0.075, 0.05)
)

# additive fit line: y = -0.1 + 0.1 * x
df_pts$y_fit <- -0.1 + 0.1 * df_pts$genotype

df_line <- data.frame(
  x = seq(0, 2, length.out = 300)
)
df_line$y <- -0.1 + 0.1 * df_line$x

# dotted horizontal guides from left margin to fitted values
df_guides <- data.frame(
  x    = c(-0.2, -0.2, -0.2),
  xend = c(0,    1,    2),
  y    = c(-0.1, 0.0,  0.1),
  yend = c(-0.1, 0.0,  0.1)
...
ggplot() +
  ...
  theme(
    ...
    axis.line.x.bottom = element_line(color = txt, linewidth = 0.7),
    axis.line.y.left   = element_line(color = txt, linewidth = 0.7),

    axis.ticks.x = element_line(color = txt, linewidth = 0.5),
    axis.ticks.y = element_line(color = txt, linewidth = 0.5),
...
<div class="r-stack" style="text-align:center;">
![Modified from James et al. (2021), An Introduction to Statistical Learning](images/gm-margi.png){width=450}
</div>
...
<div class="r-stack" style="text-align:center;">
![](images/part.png){width=700}
</div>

## Ancestral Concordance Classification

![](/images/anc-conc.png){style="max-width:100%;height:auto;"}
...
<!-- >
> <span style="font-size: 0.8em;">Tracks specific host lineage</span> -->
...
<!-- >
> <span style="font-size: 0.8em;">Does not track specific host lineages</span> -->
...
<!-- >
> <span style="font-size: 0.8em;">Initial contact of microbe and host lineage pairs</span> -->
...
> <span style="font-size: 0.7em;"> $M_L=$ </span> *lineal variance*

:::
::: {.column width="37%"}

> <span style="font-size: 0.7em;"> $M_N=$ </span>*non-lineal variance*

:::
::: {.column width="31%"}

> <span style="font-size: 0.7em;"> $M_V=$ </span>*noVel variance*
...
::: {.incremental}
- **Adaptation:**
  - Lineal: <span style="font-size: 0.8em;">$M_L$</span> *most* likely
  - Non-Lineal: <span style="font-size: 0.8em;">$M_N$</span> *less* likely
  - NoVel: <span style="font-size: 0.8em;">$M_V$</span> *no* contribution
- **How to test:**
  - Host *Pedigree*
  ...
  - Selection *Experiment*
    - Predict response including *different combinations* of <span style="font-size: 0.8em;">$M_L,M_N,M_V$</span>
...
::: {.column width="60%"}
<div class="r-stack" style="text-align:center;">
  ![Non-lineals sampled *before* selection](images/pre-selected.png)
</div>
...
::: {.column width="60%"}
<div class="r-stack" style="text-align:center;">
  ![Non-lineals sampled *after* selection](images/post-selected.png)
</div>
...
<span style="font-size: 0.8em;"> **$\pmb\Delta\mathbf{\bar z}$** </span> = Resp. to Selection, **G** = Gen. var, **L** = Lineal, **N** = Non-lineal, **V** = NoVel
...
> 
> - *<span style="font-size: 0.8em;">$M_A^\psi$</span>* = trait variance explained by variance of *selectives*
> - *<span style="font-size: 0.8em;">$C_A^\psi$</span>* = trait variance explained by covariance of *selectives* and genes
:::

::: {.fragment}
> **By definition:** (*Resp. to Sel.*) = (<span style="font-size: 0.8em;">$G_A+M_A^\psi+C_A^\psi$</span>) × (*Sel. Grad.*)
:::

::: {.fragment}
> **Without microbe fx:** (*Resp. to Sel.*) = (<span style="font-size: 0.8em;">$G_A$</span>) × (*Sel. Grad.*)
:::

::: {.fragment}
> **Narrow-sense heritability:** <span style="font-size: 0.8em;">*$h^2$* $=$</span>$\frac{G_A}{P}$, **Resp. to Sel.:** <span style="font-size: 0.8em;">$R=$ *$h^2$* $S$</span>
:::
...
::: {.fragment}
Narrow-sense **microbial transmissibility:**
$$b^2=\frac{M_A^\psi}{P}$$
...
<div class="r-stack" style="text-align:center;">
  ![Week et al. (2025), Evolution](images/qgmmt.png)
</div>
...
<div class="r-stack" style="text-align:center;">
  ![Hannah Tavalie](images/tavalire-220-x-250.png)
</div>
...
<div class="r-stack" style="text-align:center;">
  ![Peter Ralph](images/Peter-Ralph-photo-tafwfp.jpg)
</div>

:::
::: {.column width="25%"}
<div class="r-stack" style="text-align:center;">
  ![Brendan Bohanna](images/brend.jpg)
</div>
...
![Hildegard Uecker <br> MPI Plön](images/hildegard.jpg){width=300}
...
- *Birth-Death Diffusion* <span style="font-size: 0.8em;">$dn=[(r+m)\,n+\psi]\,dt+\sqrt{n}\,dB$</span>

<!-- ![](images/T-dists.png) -->

![](images/sample-path-trans.png)
...
<div class="r-stack" style="text-align:center;">
  ![Hinrich Schulenburg <br> Kiel University](images/hinrich.png)
</div>
...
- **Mesocosm experiments:**
  ...
  - Microbiome manipulation *buffers* against <span style="font-size: 0.8em;">$\Delta E$</span>
- **Quant. Gen. Theory**
  - Modify <span style="font-size: 0.8em;">*$\Delta\bar z=G\beta$*</span>
...
<div class="r-stack" style="text-align:center;">
  ![Arne Traulsen <br> MPI Plön](images/arne.webp)
</div>
...
<div class="r-stack" style="text-align:center;">
  ![Aura Raulo <br> University of Oxford](images/aura-raulo.jpg)
</div>
...
::: {.column width="49%"}
<div class="r-stack" style="text-align:center;">
  ![Pathogens accumulate in <br> clusters](images/paths.png){width=300}
</div>

:::
::: {.column width="49%"}
<div class="r-stack" style="text-align:center;">
  ![Mutualists accumulate in <br> bridges](images/muts.png){width=300}
</div>
...
:::
  ...
  <!-- - Pathogens face more resistance during transmission
  - Mutualists move relatively freely
    - Pathogens need more colonization attempts
    - Should accumulate in clustered regions
    - Mutualists accumulate in bridge nodes -->
