...
```{r, message=FALSE, warning=FALSE, tidy=TRUE, comment=''}
# read in data
load("toju_pairs.Rda")
head(toju_df)
```
...
```{r, message=FALSE, fig.align='center', fig.cap="Figure 2: Displayed in this scatter plot are mean trait pairs for various locations across Japan. The colours indicate which island these locations belong to.", tidy=TRUE, comment=''}
# plot data
library(ggplot2)
library(ggdark)
library(gapminder)
library(ggthemes)
ggplot(toju_df,aes(x=rostrum,y=pericarp,color=island))+geom_point()+dark_mode(theme_minimal())+xlab("Rostrum length")+ylab("Pericarp thickness")
```
...
```{r, message=FALSE, warning=FALSE, tidy=TRUE, tidy=TRUE, comment=''}
N   <- dim(toju_df)[1]
...
padding:15px;
margin:5px;"><b>
Finally we use the function _ml_sol()_ to find our results.
</b>
</p>
