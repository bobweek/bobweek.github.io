...
This is a tutorial for employing the method of coevolutionary inference described in [Week & Nuismer 2018](weeknuismer17.html). Data used in this tutorial is available [here](https://github.com/bobweek/inferring.coevolution). All analysis is done in the statistal programming language R.
...
ggplot(offset.data,aes(x=TONGUE.TUBE,y=PERCENT_NECTAR_CONSUMED))+
  geom_point()+stat_smooth(method = lm, formula=y~poly(x,2))+xlab("Tongue-Tube(mm)")+ylab("Percent Nectar Consumed")
```
...
ggplot(offset.data,aes(x=TONGUE.TUBE,y=POLLEN_GRAINS_DEPOSITED))+
  geom_point()+stat_smooth(method = lm, formula=y~poly(x,2))+xlab("Tongue-Tube(mm)")+ylab("Pollen Grains Deposited")
```
