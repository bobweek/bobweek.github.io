...
As you can see, we run into a bit of a snag here as the parabola is convex with the quadratic coefficient `r coeffs[3]`. This means it opens up and therefore has no maximum. If we want to find an optimal offset, we need to fit a curve that __always__ has a maximum. Gaussian curves not only satisfy this need, but they also approximate concave parabolas quite well near their maxima, making them ideal candidates to solve this problem. The expression of a typical Gaussian function is shown below.
