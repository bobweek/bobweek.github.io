...
Previous work in this area has focused on results pertaining to a small number of interacting species (cite roughgarden, abrams, holt and mcpeek). However, many of the patterns observed span dozens, even hundreds of species. To overcome this a recent development in coevolutionary theory has taken the so-called mean field approach to this problem (cite paper that's in review and scott's 2013 paper). This approach allows the reduction of a high-dimensional problem to a low-dimensional one at the cost of resolution. With this approach we can derive analytical results pertaining to the statistical nature of communities as whole, but without explicit conclusions for each member of the community. Hence, we are now in a position to develop a mathematically rigorous theory of coevolutionary community ecology. The primary goal of this work is to contribute to this development by extending previous work to the context of mutualistic communities. 

However, there are two major steps to be addressed before such a contribution can be made. First, a more biologically realistic treatment of the mechanisms mediating the outcomes of ecological interactions needs to be incorporated into coevolutionary theory. This alone may explain a significant amount of variability seen in nature. Second, a metric of the strength of coevolution within a community is required to quantify the role of coevolution in shaping communities. To derive this community-wide metric from first principles we will first need a way to measure the strength of coevolution between a pair of species (hereon referred to as pairwise coevolution). To tether this work to the empirical world, we motivate each step by the details of specific pollination networks. The three primary questions driving this work are:
...
We know almost nothing about the strength and prevalence of coevolution in the wild. This is largely due to the lack of generally applicable statistical approaches that yield quantitative estimates. Here I propose a novel method that derives maximum likelihood estimates for the strength of pairwise coevolution in natural systems by coupling the coevolutionary model described above to widely available phenotypic data. I will apply this method to two well-studied interactions; one being a highly specific antagonism involving the Japanese camellia fruit and its predatory weevil and the other the mutualism previously discussed between the long-tongued fly M. longirsotris and the flower it pollinates L. anceps. Broad application of this approach has the potential to answer long-standing evolutionary debates such as the importance species interactions play in the evolution of sexual reproduction.
...
A promising alternative to existing approaches is the development of a likelihood-based framework that uses routinely collected phenotypic data to estimate key parameters of the offset-matching coevolutionary model and to test the hypothesis of coevolution. Specifically, the offset-matching model can be easily extended to predict the spatial distribution of traits for a pair of interacting species that evolve in response to random genetic drift, abiotic selection, and coevolution  [@nuismer2010correlation]. Crucially, this extension predicts that the distribution of trait means in the interacting species across a metapopulation will approach a bivariate normal distribution entirely described by five statistical moments: the average value of the key trait in each species among populations, the variance of the key trait in each species among populations, and the spatial association (covariance) between the key traits in each species. Because the models predict a bivariate normal distribution of traits, calculating the likelihood of observing any particular set of trait values in a pair of interacting species is straightforward (SI). Maximizing the resulting likelihood with respect to the key model parameters that quantify the strength of biotic selection acting on the two species ($B_1,B_2\geq0$) and two parameters that capture any selective agents acting independently of the interaction (the strengths of abiotic selection $A_1,A_2\geq0$) yields a quantitative estimate for the strength of coevolution under the assumption that the remaining model parameters have been accurately estimated (SI).
...
## Q3: What is the role of coevolution in structuring mutualistic communities?
  ...
  * Why do some communities have consistently exaggerated traits, others have consistently modest traits and yet other communities have a wide range of trait values?
  * Why do some species have exaggerated traits and yet only weakly coevolve with any other species?
  * Is it due to the network structure of the community?
  * Will modularity increase diversity?
