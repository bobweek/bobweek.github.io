...
The parameters I anticipate to be most important in determining when specialization will occur are the strengths of coevolution ($B$) and the optimal offset ($\delta$). By imposing specific statistical distributions on these parameters across the community, we might find, for example, that a high variance in $\delta$ inevitably leads to the specialization in interactions between a subset of the community. This analytical approach will also be complemented by a set of simulations that probe the less tractable situations that arise when assumptions are broken.
