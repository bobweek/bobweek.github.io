...
$$\bar Z_t(I)\equiv\frac{1}{b-a}\int_a^b\bar z(x,t)dx.$$
...
$$\bar Z_t(A)\equiv\frac{1}{|A|}\int_A\bar z(x,t)dx.$$

where $|A|=\sum_i|I_i|=\sum_i(b_i-a_i)$ denotes the Lebesgue measure (the sum of the lengths of component intervals). 

Under diffusive migration with rate $m$ and random genetic drift with variance $\eta$ the evolution of $\bar z(x,t)$ is given by

$$d\bar z=m\frac{\partial^2\bar z}{\partial x^2}dt+\eta\dot W$$
...
$$d\bar z_1=\left\{G_1M_1(\bar z_2-\bar z_1)+G_1D_1+m_1\frac{\partial^2\bar z_1}{\partial x^2}\right\}dt+\eta_1\dot W_1$$

$$d\bar z_2=\left\{G_2M_2(\bar z_1-\bar z_2)-G_2D_2+m_2\frac{\partial^2\bar z_2}{\partial x^2}\right\}dt+\eta_2\dot W_2$$
...
The global mean trait of species $i$ at time $t$ is given by

$$\mu_i(t)\equiv\lim_{\xi\rightarrow\infty}\frac{1}{2\xi}\int_{-\xi}^\xi\bar z_i(x,t)dx$$

and the variance in local mean traits by

$$V_i(t)\equiv\lim_{\xi\rightarrow\infty}\frac{1}{4\xi^2}\int_{-\xi}^\xi(\bar x_i(x,t)-\mu_i(t))^2dx$$

and the covariance by

$$C(t)\equiv\lim_{\xi\rightarrow\infty}\frac{1}{4\xi^2}\int_{-\xi}^\xi(\bar x_1(x,t)-\mu_1(t))(\bar x_2(x,t)-\mu_2(t))dx.$$

Hence, the correlation can by tracked by computing $\rho(t)\equiv\frac{C(t)}{\sqrt{V_1(t)V_2(t)}}$.
