...
Returning to the point being made above, for any pair of interacing species $(i,j)$ the probaility that either $\mathcal{B}_{ij}$ or $\mathcal{B}_{ji}$ are equal to zero is zero. Using the same argument for the magnitudes of $G_i$ and $G_j$, then we will always find at least some $\epsilon$-amount of coevolution for any given pair of interacting species. This is fine; it should be understood that coevolution is continuous in value, not binary. Also, the triviality of this notion of coevolution does not remove its relevance, but emphasizes the need to measure it. I define the __strength of coevolution__ between species $i$ and $j$ as $C_{ij}=G_iG_j|\mathcal{B}_{ij}\mathcal{B}_{ji}|$. The distribution of this metric in a community can provide important information detailing the structure of the community. From this metric we can construct other measurements that provide insight into specific coevolutionary structures of communities. For example suppose species $i$ interacts with the set $I_i$ of species and $j$ interacts with $I_j$. Set $\gamma_i=\argmax_{l\in I_i}\{C_{il}\}$. If $\gamma_i=j$ and $\gamma_j=i$ then coevolution is strongest between species $i$ and $j$ than between $i$ and any of its partners other than $j$ and likewise for $j$. As this particular strength of coevolution increases above the rest, a pairwise coevolutionary situation emerges. 

Once again, however, there is no threshold from which the coevolutionary relationship between this pair of species immediately transitions to pairwise. Setting $\lambda_i=\argmax_{l\in I_i\setminus\{\gamma_i\}}\{C_{il}\}$, one potential measure of pairwise coevolution is $C^2_{ij}-C_{i\lambda_i}C_{j\lambda_j}$. But this would only make sense when $\gamma_i=j$ and $\gamma_j=i$. Instead we need a notion that describes the degree to which a species is uniquely coevolving with another species. I call this __evolutionary affinity__ and for species $i$ it can be captured by $\phi_i\equiv C^2_{i\gamma_i}-C_{i\lambda_i}C_{\gamma_i\lambda_{\gamma_i}}$ (note that when $\phi_i=\phi_{\gamma_i}$ this corresponds to a measure of pairwise coevolution since it implies that $\gamma_i=j$ and $\gamma_j=i$). The subset of coevolving pairs $(i,j)$ for which $\gamma_i=j$ and $\gamma_j=i$ are an important subset of interactions for if there are any pairs that are truly undergoing pairwise coevolution, they will be in this set. For theses cases I'll write $\phi_{ij}$ instead of $\phi_i$ or $\phi_j$ to emphasize the pairwise relationship between species $i$ and $j$. Denoting $\Theta$ as the set of all pairs of coevolving species and $\Pi\subset\Theta$ as the set of potentially pairwise coevolving pairs (those for which $\phi_i=\phi_j=\phi_{ij}$) we can provide a cursory definition of the __pairwisivity__ of a community as

$$\pi=\frac{|\Pi|}{|\Theta|}\sum_{(i,j)\in\Pi}\phi_{ij}.$$

This quantity is interesting for it allows us to investigate the distinction between diffuse coevolution and pairwise coevolution. The case of diffuse coevolution is best described as a coevolving community with $\pi=0$. However, if there's any variability in the strength of coevolution across the community, the pair with the greatest strength of coevolution will be sure to be an element of $\Pi$. Hence, $\pi>0$ a.s. As $\pi$ increases the community tends to be more populated with pairwise coevolutionary relationships.
...
#### Here's an example of a coevolving community with high parwisivity.
...
pi <- (nrow(Pi)/nedge)*sum(w[sg])
hist(w,xlab="Coevolution",main="")
```

In this case we get $\pi=$ `r pi`.

#### Here is a coevolving community that is completely diffuse.
...
#### Here is a coevolving community that is still more diffuse but has some pairwisivity
...
The for a community that has been freshly disturbed and time $t=0$, we might expect to see pairwisivity ($\pi$) take the following trajectory.

```{r, include=F}
s <- 1
nums <- seq(0,100,0.1)
pi <- 30/(1+exp((nums-50)/10))
```

```{r, echo=F}
plot(nums,pi,xlab="time",ylab="pairwisivity")
```
