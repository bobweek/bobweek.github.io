...
def ellipse_axes(M):
    ...
    return 2*np.sqrt(w[1]), 2*np.sqrt(w[0]), angle, V[:, 1]   # +major eigenvector
...
plt.rcParams.update({"axes.edgecolor": SUBTLE, "text.color": TXT,
                     "figure.facecolor": BG, "axes.facecolor": BG, "axes.labelcolor": TXT})
fig, axes = plt.subplots(2, 3, figsize=(9, 6)); LIM = 1.6
ells, lines = [], []
for col in range(3):
    for row in range(2):
        ax = axes[row, col]
        ...
        el = Ellipse((0, 0), 1, 1, fc=REPCOL[col], ec=REPCOL[col], alpha=0.4, lw=2)
        ax.add_patch(el); ells.append(el)
        ln, = ax.plot([], [], color=REPCOL[col], lw=1.2, ls=(0, (5, 4)), alpha=0.55)
        lines.append(ln)
axes[0, 0].set_ylabel("G-matrix", fontsize=12)
axes[1, 0].set_ylabel("correlation matrix", fontsize=12)
fig.tight_layout()

# fixed-length guide line: shows the (major-axis) collapse DIRECTION, so it stays
# visible even after the G-oval shrinks toward a point. The G-row line points in a
# random direction per replicate; the correlation-row line always sits at +/-45 deg.
L = 1.15
def update(fi):
    k = frames[fi]; idx = 0
    for col in range(3):
        ...
        for M in (G, correlation(G)):
            w, h, a, vec = ellipse_axes(M)
            ells[idx].width, ells[idx].height, ells[idx].angle = w, h, a
            lines[idx].set_data([-L*vec[0], L*vec[0]], [-L*vec[1], L*vec[1]])
            idx += 1
    return ells + lines
