...
def render(hist, out_path, emphasis_gen=None, fps=14, hold_ms=1700,
    ...
    # Adaptive sampling: place frames where the correlations are actually
    # MOVING.  Uniform-in-time sampling skips ~n_gen/max_frames generations per
    # frame, so during the rapid collapse a line can travel far between two shown
    # frames and look like it "jumps" to +/-1.  We instead sample uniformly in
    # cumulative correlation change, so the busy window gets most of the frames.
    if n_gen + 1 > max_frames:
        d = np.abs(np.diff(Cfull, axis=0))            # (n_gen, n_pairs)
        act = np.nansum(d, axis=1) + 1e-9             # per-gen total movement
        cum = np.concatenate([[0.0], np.cumsum(act)])
        cum /= cum[-1]
        targets = np.linspace(0, 1, max_frames)
        idx = {int(np.searchsorted(cum, tt)) for tt in targets}
    else:
    ...
    def draw_corr(gen):
        ...
        if fold:
            ...
            ylab = ("Genetic correlation  \u03c1" if ref_locus is None
                    else f"\u03c1 with locus {ref_locus + 1}")
    ...
    def draw_frame(gen):
        ...
        hdr = "CORRELATIONS" if ref_locus is None else f"CORRELATIONS vs locus {ref_locus + 1}"
        ...
        tag = "fixed \u2014 one haplotype" if nh == 1 else f"mean |\u03c1| = {mean_abs:0.2f}"
    ...
    for gen in idx:
        ...
        d = int(1000 / fps)
        if emphasis_gen is not None and int(gen) == int(emphasis_gen):
            d = emphasis_ms
        durations.append(d)
    plt.close(fig)

    durations[-1] = hold_ms                              # hold the final state
...
def main():
    ...
    FOLD_ABS  = False         # True -> plot |rho| on a 0..1 half-axis (no +/- mirror)
    ...
    if RUN_TO == "money_shot" and g_money is not None:
        render(hist[: g_money + 1], "wf_correlations.gif", emphasis_gen=None,
               fold=FOLD_ABS, ref_locus=ref_locus)
    else:                                                # run to final time
        render(hist, "wf_correlations.gif", emphasis_gen=g_money,
               fold=FOLD_ABS, ref_locus=ref_locus)
